...
# h5py数据生成器
def h5_generator(inputPath, tfidfinputPath,all_num , batch_size, mode="train"):
    ...
    while True:
        ...
        if mode=='train':
            ...
            for i in range(batch_size):
                ...
                if num > beta:
                    ...
                    state = np.random.get_state()   #同步打乱机制
        ...
        yield ([input1, input2,input3,input4,input5,input6], labels)


train_vail = np.zeros((600000, 10))
sub = np.zeros((1000000, 10))
score = []
if not os.path.exists("./model/age_model_1"):
    os.mkdir("./model/age_model_1")
...
for i in range(num):
    ...
    print("###"*35)
    gc.collect()
    filepath = f"./model/age_model_1/nn_v1_{i}.h5"
    ...
    model_age = model_conv(emb1,emb2,emb3,emb4,emb5,252)
    #if count==0:
    model_age.summary()
    ...
    vail_batch = 5000
    test_batch = 5000
    trainGen = h5_generator(inputPath=filepath_tr,tfidfinputPath=filepath_tr_tf,all_num = 2400000, batch_size=train_batch, mode="train")
    vailGen = h5_generator(inputPath=filepath_te,tfidfinputPath=filepath_te_tf,all_num=600000, batch_size=vail_batch, mode="vail")
    testGen = h5_generator(inputPath='w2v_feature/test.h5',tfidfinputPath='tfidf/test.h5', all_num=1000000, batch_size=test_batch, mode="test")
