...
def model_conv(emb1, emb2,emb3,emb4,emb5,emb6,emb7,emb8,emb9,emb10,num_feature_input):
    ...
    semantic2 = TimeDistributed(Dense(128, activation="relu"))(semantic)
    semantic2 = TimeDistributed(Dense(128, activation="relu"))(semantic2)
    semantic2 = Add()([semantic,semantic2])
    semantic3 = Concatenate()([semantic,semantic2])
    merged_1 = Lambda(lambda x: K.max(x, axis=1), output_shape=(256,))(semantic3)
    merged_1_avg = Lambda(lambda x: K.sum(x, axis=1)/input_mask, output_shape=(256,))(semantic3)
    ...
    x = Concatenate()([merged_1,merged_1_avg,stat_fea,stat_fea2]) 
...
score = []
if not os.path.exists("./model/age_model_5"):
    os.mkdir("./model/age_model_5")
...
for i in range(num):
    ...
    print("###"*35)
    gc.collect()
    filepath = f"./model/age_model_5/nn_v1_{i}.h5"
    filepath_tr = f"./w2v_feature/nn_feature{i}/train{ii}.h5"
