...
def model_conv(emb1, emb2,emb3,emb4,emb5,num_feature_input):
    ...
    lstm1 = Dropout(0.35)(Bidirectional(CuDNNLSTM(256, return_sequences=True))(x11))

    gru1 = Dropout(0.35)(Bidirectional(CuDNNGRU(256, return_sequences=True))(lstm1))

    cnn1d_layer=keras.layers.Conv1D(128, kernel_size=1, padding="valid", kernel_initializer="he_uniform")

    #lstm1 = lstm_layer(x11)
    #gru1 = gru_layer(lstm1)
    cnn1 = cnn1d_layer(lstm1)

    fea = Concatenate()([lstm1,gru1,cnn1])
    

    semantic = TimeDistributed(Dense(512, activation="relu"))(fea)
    semantic = TimeDistributed(Dense(256, activation="relu"))(semantic)
    merged_1 = Lambda(lambda x: K.max(x, axis=1), output_shape=(256,))(semantic)
    merged_1_avg = Lambda(lambda x: K.mean(x, axis=1), output_shape=(256,))(semantic)


    att_1 = Attention(100)(lstm1)
    att_3 = Attention(100)(gru1)
    att = Concatenate()([att_1,att_3])
    att = Dense(256, activation='relu')(att)
    att = Dense(128, activation='relu')(att)

    
    hin = Input(shape=(num_feature_input,))
    htime = Dense(256, activation='relu')(hin)
    htime = Dense(128, activation='relu')(htime)
    
    x= Concatenate()([att,merged_1,merged_1_avg,htime])
    
    x = Dropout(0.4)(Activation(activation="relu")(BatchNormalization()(Dense(1024)(x))))
    x = Activation(activation="relu")(BatchNormalization()(Dense(512)(x)))
    pred = Dense(2, activation='softmax')(x)
    model = Model(inputs=[seq1,seq2,seq3,seq4,seq5,hin], outputs=pred)
...
def h5_generator(inputPath, tfidfinputPath,countinputPath,all_num , batch_size, mode="train"):
    ...
    while True:
        ...
        yield ([input1, input2,input3,input4,input5,input8], labels)


train_vail = np.zeros((600000, 2))
sub = np.zeros((1000000, 2))
score = []
if not os.path.exists("./model/gender_model_3"):
    os.mkdir("./model/gender_model_3")
...
for i in range(num):
    ...
    print("###"*35)
    gc.collect()
    filepath = f"./gender_model_3/nn_v1_{i}.h5"
